...
for iterations in os.listdir("results"):
    for lam in os.listdir("results/"+iterations):
        for p in os.listdir("results/{}/{}".format(iterations, lam)):
            df = pd.read_csv("results/{}/{}/{}/loss.csv".format(iterations, lam, p))
            fig.add_trace(go.Scatter(x = df['epoch'], y = df['loss'], name="{}/{}/{}".format(iterations, lam, p)))
